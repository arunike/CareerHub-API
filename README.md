...
- **Food on office days**: `Application.free_food_meals` holds one entry per meal — `[{meal, value, provided}]` — because a $6 breakfast and a $20 dinner should not average into one figure, and because a meal the office does **not** provide is money you spend rather than a zero. The annual figure is `provided × officeDays − youPay × officeDays`, using the same office-day count the commute uses, so it can be negative and a two-day hybrid is not credited with five days of free lunches. `free_food_value_per_meal` is retained only to migrate the earlier shape (a plain list of meal keys at one shared value), which is read as all-provided. The legacy `free_food_perk_value`/`free_food_perk_frequency` columns are kept and still read as a fallback when no meals are set, so offers saved before this keep their value instead of silently reading as zero. Migration `0030` adds both columns with `SeparateDatabaseAndState` and raw `ADD COLUMN`, since a generated `AddField` with a default emits an `ALTER COLUMN … DROP DEFAULT` this Postgres rejects
- **Response trend**: `response_trend` compares two 30-day cohorts of applications, both ending at today minus the p90 reply time so each has had the same chance to answer. Cohort membership is keyed on `date_applied`, **not** the APPLIED timeline entry: on synced rows that entry drifts from the date the user set (404 of 806 differ, typically by 4–7 days, and 133 have no entry at all), which moves applications between windows and can invert the sign. Withheld entirely below `MIN_TREND_COHORT` applications per window, because a delta off a handful is noise. Pinned by `ResponseTrendTests`, including a case that fails if cohorts are keyed off the timeline entry
...
- **Reply timing**: `/career/application-timeline-analytics/` returns `response_time_buckets` (0-7 / 8-14 / 15-30 / 31-60 / 60+ days with cumulative share), `median_days_to_response`, `p90_days_to_response`, and `suggested_followup_days`. A reply is any movement past applied, read from the timeline rather than status, so an application that interviewed and was then rejected still counts as answered. `suggested_followup_days` is the p90 taken from the distribution rather than snapped to a bucket edge, and `silent_past_followup_count` counts still-open applications already past it
...
- **Driving Defaults** (`default_mpg`, `default_gas_price_per_gallon`, migration `0011`): Your car's mileage and your pump price, stored once per user because they do not change per offer. Every offer's fuel-costed commute reads them; an offer stores a value in its own `commute_options` row only when it genuinely differs, so a gas-price change reprices all offers at once instead of needing each one edited. Defaults are 28 mpg and $4.00/gal, and the columns are added with the same `SeparateDatabaseAndState` + raw `ADD COLUMN` pattern the other defaulted fields use
