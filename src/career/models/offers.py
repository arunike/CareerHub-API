...
class Offer(models.Model):
    ...
    pto_rollover_max_days = models.PositiveSmallIntegerField(default=0, help_text="Unused days that carry into next year. 0 means none carry.")
    pto_rollover_expires_month = models.PositiveSmallIntegerField(default=0, help_text="Month the carried days lapse, 1-12. 0 means they never lapse.")
    pto_accrual_days_per_year = models.PositiveSmallIntegerField(default=0, help_text="Extra days granted per completed year of service. 0 means the allowance never grows.")
    pto_accrual_max_days = models.PositiveSmallIntegerField(default=0, help_text="Ceiling on the days tenure can add. 0 means no ceiling.")
    pto_hours_per_day = models.DecimalField(max_digits=4, decimal_places=2, default=8, help_text="Hours one PTO day is worth, for policies quoted in hours")
